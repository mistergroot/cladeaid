...
def add_arguments(parser):
    ...
    parser.add_argument("--max_reads", type=int, help="Maximum number of reads to process")
    ...
    parser.add_argument("--extract_read_attributes", action="store_true", help="Extract and output read attributes")
...
def run(args):
    ...
    with open(args.output + ".csv", "w", newline="") as f:
        ...
        if args.extract_read_attributes:
            writer.writerow([
                "ReadName", "TaxID", "TaxName", "Rank", "TotalBases", "BestIdentity",
                "ReadPairConcordance", "R1_Length", "R1_Matches", "R1_Insertions",
                "R1_Deletions", "R1_Softclips", "R1_Hardclips", "R1_AvgQual",
                "R2_Length", "R2_Matches", "R2_Insertions", "R2_Deletions",
                "R2_Softclips", "R2_Hardclips", "R2_AvgQual"
            ])
        else:
            writer.writerow([
                "ReadName", "TaxID", "TaxName", "Rank", "TotalBases", "BestIdentity",
...
def main():
    parser = argparse.ArgumentParser(description="Assign taxonomic labels using LCA + specificity.")
