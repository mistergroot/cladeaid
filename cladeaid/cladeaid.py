...
def main():
    ...
    parser.add_argument("--pairwise_dists", default="all.dists",
                        help="Optional - Path to a list of pairwise " \
                        "distances between all genomes to use for mash " \
                        "reallocation of bases. If file does not exist, "
                        "if will be computed for you.")
    parser.add_argument("--multifasta", default=False, action="store_true", 
                        help="Optional - If the bam files were generated by " \
                        "mapping data against a fasta that contains multiple" \
                        " organisms, this flag must be enabled. Using this " \
                        "option assumes that each organism/individual is " \
                        "represented by a single sequence in the multifasta " \
                        "reference. The default assumption is that you've " \
                        "mapped the same dataset against multiple references" \
                        " separately, and then concatenated the bam files " \
                        "afterwards.")
    parser.add_argument("--normalize", action="store_true",
                        help="Optional - Normalize bases assigned to " \
                        "different taxa based on their genome sizes. " \
                        "This is highly recommended if you are working with " \
                        "anything other than mock mixes. This should be used"
                        " if you are working with any sample of mixed origin" \
                        " that was already a blend of organisms prior to " \
                        "extraction. I would also disable this for anything " \
                        "where you expect copy number variation per cell "
                        "(e.g. mitochondrial data)")
    ...
    if args.estimate_abundance:
        ...
        assignments['Assigned_Bases'] = (assignments
                                         .groupby(['TaxID', 'TaxName'])
        ...
        if args.mash_reallocation:
            ...
            grouped_full = grouped_rows.T.groupby(distmatrix.columns, 
                                                  axis=0).mean().T
        ...
        naive_abundances.columns = ["TaxID", "Naive_Bases"]
        penalized_abundances = pd.DataFrame(penalized_abundances.items())
        penalized_abundances.columns = ["TaxID", "Penalized_Bases"]
        propagated_counts = (assignments[["TaxID", "TaxName", "Rank", 
                                          "Assigned_Bases"]]
                                          .merge(naive_abundances, 
                                                 how = "left", on = "TaxID")
                                                 .merge(penalized_abundances,
                                                        how = "left", 
                                                        on = "TaxID"))
        ...
        if args.normalize: 
            ...
            reference_sizes = (reference_sizes.groupby(0)[1]
                               .mean().reset_index())
            ...
            for prop in ["Naive", "Penalized"]:
                ...
                for i in range(len(propagated_counts)):
                    if not np.isnan(propagated_counts[prop + "_Bases"][i]):
                        propagated_counts[prop + "_Proportion"][i] = (
                            propagated_counts[prop + "_Bases"][i] / 
                            reference_sizes[propagated_counts["TaxID"][i]])
